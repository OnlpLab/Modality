...
for exp in ['only_target_head_five']:
    for split in [0, 1, 2, 3, 4]:
        try:
            ...
            subprocess.run(command, check=True, shell=True)
        try:
            command = "allennlp predict probing_exp/"+exp+str(split)+"/model.tar.gz data/"+str(split)+"/dev_prejacent_five_head.txt --include-package my_library --use-dataset-reader --cuda-device 1 --output-file probing_predictions2/"+exp+str(split)
            subprocess.run(command, check=True, shell=True)
        except subprocess.CalledProcessError:
            command = "allennlp predict probing_exp/"+exp+str(split)+"/model.tar.gz data/"+str(split)+"/dev_prejacent_five_head.txt --include-package my_library --use-dataset-reader --cuda-device 1 --output-file probing_predictions2/"+exp+str(split)
            subprocess.run(command, check=True, shell=True)
        try:
            command = "allennlp predict probing_exp/"+exp+str(split)+"/model.tar.gz data/full/test_prejacent_five_head.txt --include-package my_library --use-dataset-reader --cuda-device 1 --output-file probing_predictions2/test_"+exp+str(split)
            subprocess.run(command, check=True, shell=True)
        except subprocess.CalledProcessError:
            command = "allennlp predict probing_exp/"+exp+str(split)+"/model.tar.gz data/full/test_prejacent_five_head.txt --include-package my_library --use-dataset-reader --cuda-device 1 --output-file probing_predictions2/test_"+exp+str(split)
            subprocess.run(command, check=True, shell=True)

        sent_file = "data/"+str(split)+"/dev_prejacent_five_head.txt"
        pred_file = "probing_predictions2/"+exp+str(split)
        outfile = "probing_predictions2/readable_"+exp+str(split)
        convert_prediction(sent_file, pred_file, outfile)

        sent_file = "data/full/test_prejacent_five_head.txt"
        pred_file = "probing_predictions2/test_"+exp+str(split)
        outfile = "probing_predictions2/readable_test"+exp+str(split)
        convert_prediction(sent_file, pred_file, outfile)
