...
for exp in ['classifier_with_target_binary']:
    for split in [0, 1, 2, 3, 4]:
        try:
            ...
            subprocess.run(command, check=True, shell=True)
        try:
            command = "allennlp predict probing_exp/"+exp+str(split)+"/model.tar.gz data/"+str(split)+"/dev_prejacent_binary_head.txt --include-package my_library --use-dataset-reader --cuda-device 2 --output-file probing_predictions2/"+exp+str(split)
            subprocess.run(command, check=True, shell=True)
        except subprocess.CalledProcessError:
            command = "allennlp predict probing_exp/"+exp+str(split)+"/model.tar.gz data/"+str(split)+"/dev_prejacent_binary_head.txt --include-package my_library --use-dataset-reader --cuda-device 2 --output-file probing_predictions2/"+exp+str(split)
            subprocess.run(command, check=True, shell=True)
        try:
            command = "allennlp predict probing_exp/"+exp+str(split)+"/model.tar.gz data/full/test_prejacent_binary_head.txt --include-package my_library --use-dataset-reader --cuda-device 2 --output-file probing_predictions2/test_"+exp+str(split)
            subprocess.run(command, check=True, shell=True)
        except subprocess.CalledProcessError:
            command = "allennlp predict probing_exp/"+exp+str(split)+"/model.tar.gz data/full/test_prejacent_binary_head.txt --include-package my_library --use-dataset-reader --cuda-device 2 --output-file probing_predictions2/test_"+exp+str(split)
            subprocess.run(command, check=True, shell=True)

        sent_file = "data/"+str(split)+"/dev_prejacent_binary_head.txt"
        ...
        sent_file = "data/full/test_prejacent_binary_head.txt"
