...
from my_library.metrics.tagging_accuracy import TaggingAccuracy


@Model.register("simple_tagger_mod")
class SimpleTagger(Model):
    ...
    def forward(
        ...
        output_dict = {"logits": logits, "class_probabilities": class_probabilities, "gold_labels": [x["target_tags"] for x in metadata]}
        ...
        if tags is not None:
            ...
            if self.downsample:
                o_tag_index = self.vocab.get_token_index("X", namespace=self.label_namespace)
                tag_mask = mask & (tags != o_tag_index)
            else:
                tag_mask = mask
            ...
            prediction = output_dict["class_probabilities"]
            prediction = self.get_human_readable(prediction)
            tags_labels = [x["target_tags"] for x in metadata]
            self._mod_acc(prediction, tags_labels)
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        mod_accuracy = self._mod_acc.get_metric(reset=reset)
        metrics_to_return.update(mod_accuracy)
        if self.calculate_span_f1:
