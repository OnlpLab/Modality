...
@Model.register("crf_tagger_mod_ind")
class CrfTagger(Model):
    ...
    def forward(
        ...
        **kwargs,  # to allow for a more general dataset reader that passes args we don't need
        ...
        """
        embedded_input = self.text_field_embedder(tokens)
        ...
        output = {"logits": logits, "mask": mask, "tags": predicted_tags, "gold_labels": [x["target_tags"] for x in metadata]}
        ...
        if tags is not None:
            ...
            prediction = self.get_human_readable(output["tags"])
    ...
    def get_human_readable(
        ...
        new_tags = [decode_tags(t) for t in tags]
