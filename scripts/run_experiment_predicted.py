...
for exp in ['triggering_target_five_for_pred']:
    for split in [0, 1, 2, 3, 4]:
        try:
            ...
            subprocess.run(command, check=True, shell=True)
        try:
            command = "allennlp predict probing_exp/"+exp+str(split)+"/model.tar.gz data/full/test_prejacent_predicted_modal_five0.txt --include-package my_library --use-dataset-reader --cuda-device 3 --output-file probing_predictions3/test_with_predicted"+exp+str(split)
            subprocess.run(command, check=True, shell=True)
        except subprocess.CalledProcessError:
            command = "allennlp predict probing_exp/"+exp+str(split)+"/model.tar.gz data/full/test_prejacent_predicted_modal_five0.txt --include-package my_library --use-dataset-reader --cuda-device 3 --output-file probing_predictions3/test_with_predicted"+exp+str(split)
            subprocess.run(command, check=True, shell=True)
        ...
        pred_file = "probing_predictions3/test_with_predicted"+exp+str(split)
        outfile = "probing_predictions3/readable_test_with_predicted"+exp+str(split)
