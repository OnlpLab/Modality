...
for exp in ['full_classifier_fine']:
    for split in [0, 1, 2, 3, 4]:
        try:
            ...
            subprocess.run(command, check=True, shell=True)
        try:
            command = "allennlp predict probing_exp/"+exp+str(split)+"/model.tar.gz data/all_unrolled_no_ambiguities/"+str(split)+"/dev_all_unrolled_no_ambiguities_space.txt --include-package my_library --use-dataset-reader --cuda-device 1 --output-file probing_predictions/"+exp+str(split)
            subprocess.run(command, check=True, shell=True)
        except subprocess.CalledProcessError:
            command = "allennlp predict probing_exp/"+exp+str(split)+"/model.tar.gz data/all_unrolled_no_ambiguities/"+str(split)+"/dev_all_unrolled_no_ambiguities_space.txt --include-package my_library --use-dataset-reader --cuda-device 1 --output-file probing_predictions/"+exp+str(split)
            subprocess.run(command, check=True, shell=True)
        try:
            command = "allennlp predict probing_exp/"+exp+str(split)+"/model.tar.gz data/all_unrolled_no_ambiguities/test_all_unrolled_no_ambiguities_space.txt --include-package my_library --use-dataset-reader --cuda-device 1 --output-file probing_predictions/test_"+exp+str(split)
            subprocess.run(command, check=True, shell=True)
        except subprocess.CalledProcessError:
            command = "allennlp predict probing_exp/"+exp+str(split)+"/model.tar.gz data/all_unrolled_no_ambiguities/test_all_unrolled_no_ambiguities_space.txt --include-package my_library --use-dataset-reader --cuda-device 1 --output-file probing_predictions/test_"+exp+str(split)
            subprocess.run(command, check=True, shell=True)

        sent_file = "data/all_unrolled_no_ambiguities/"+str(split)+"/dev_all_unrolled_no_ambiguities_space.txt"
        pred_file = "probing_predictions/"+exp+str(split)
        outfile = "probing_predictions/readable_"+exp+str(split)
        convert_prediction(sent_file, pred_file, outfile)

        sent_file = "data/all_unrolled_no_ambiguities/test_all_unrolled_no_ambiguities_space.txt"
        pred_file = "probing_predictions/test_"+exp+str(split)
        outfile = "probing_predictions/readable_test"+exp+str(split)
        convert_prediction(sent_file, pred_file, outfile)
